...
class GetImgAPI:
    def __init__(self, master: tkinter.Tk = None):
        ...
        if type(master) != tkinter.Tk:
            ...
            if not exists(f"{main_path}/Data/img/icon/icon.ico"):
                with open(
                    file=f"{main_path}/Data/img/icon/icon.ico", mode="wb"
                ) as save_icon_file, urlopen(
                    url="https://icon-icons.com/downloadimage.php?id=213814&root"
                    "=3392/ICO/64/&file=python_icon_213814.ico"
                ) as get_icon_file:
        ...
        # after
        self.the_img_weg.after(ms=self.switch_interval * 1000, func=self.auto_switch)
    ...
    def create_work_dirs():
        ...
        def _if_not_exists_json_file_mk(json_path, init_json_data):
            if not exists(json_path):
                with open(
                    file=json_path, mode="w", encoding="UTF-8", newline="\n"
                ) as config_file:
                    dump(
                        fp=config_file, obj=init_json_data, indent=4, ensure_ascii=False
                    )
        ...
        _if_not_exists_json_file_mk(
            f"{main_path}/Data/config/setu-r18.json", setu_r18_config
        )
    ...
    def wallpaper(self):
        new_image = Image.new(
            ...
            (
                ...
                if type(self.wallpaper_bg_color) == tuple
                else tuple(self.wallpaper_bg_color)
            ),
        )  # 生成纯色图像
        # // 为整数除法，计算图像的位置
        new_image.paste(
            self.the_img, self.show_w_h
        )  # 将图像填充为中间图像，两侧为黑色的样式
    ...
    def settings_refresh(self):
        with open(
            file=f"{main_path}/Data/config/settings.json", mode="r", encoding="UTF-8"
        ) as set_jf:
    ...
    def save_settings(self):
        ...
        with open(
            file=f"{main_path}/Data/config/settings.json",
            mode="w",
            encoding="UTF-8",
            newline="\n",
        ) as set_jf:
    ...
    def get_image(
        ...
        url_: str = None,
        path: str = None,
        request_data_: dict = None,
        response_processing: str = None,
        ...
        if url_ is not None:
            ...
            if request_data_ is not None:
                ...
                while not ok:
                    ...
                    if retry_count <= self.retry_count_max:
                        ...
                        self.log_out(
                            "<!> 重试连接次数过多已切换至下条链接", level="WARNING"
                        )
                        return None
            # 如果请求数据为空
            else:
            ...
            if response.headers["content-type"] == "application/json; charset=utf-8":
                ...
                if response_processing:
                    ...
                    response = get(url=img_url, proxies=self.proxies, headers=headers)
            ...
            old_content_file_path = (
                "{}/Data/img/temp/response/response_content.{}".format(
            ...
            if self.file_suffix.upper() not in self.support_type:
                self.log_out(
                    "<!> 返回数据类型 {} 非图片格式".format(self.file_suffix),
                    level="ERROR",
                )
                return None
        ...
        image = self.original_img.resize((w_size, h_size), resample=self.img_load_mode)
    ...
    def configure_image(self):
        def __configure_image():
            ...
            self.get_save_img(
                ...
                response_processing=self.response_processing,
            ...
            if exists(self.now_file_path):
                ...
                if exists(ask_img_file_path):
                    ...
                    if ask_img_file_path == "":
                        self.log_out(
                            "<!> 已取消选择图片, 切换至网络模式!", level="WARNING"
                        )
                    else:
                        self.log_out(
                            "<!> 选择的图片不存在, 切换至网络模式!", level="ERROR"
                        )
                    self.acquired_mode = "web"
    ...
    def save_img(self):
        ...
        save_file_path = asksaveasfilename(
            filetypes=[("PNG", "*.png"), ("JPG", "*.jpg"), ("任意图片格式", "")],
    ...
    def history_in(self):
        ...
        def __del_extra_history():
            while self.history_count_num > self.history_count_max:
                extra_img_path = join(
                    self.history_save_dir, listdir(self.history_save_dir)[0]
                )
    ...
    def set_up(self):
        ...
        def set_switch_interval():
            ...
            if self.is_int(ent_switch_interval):
                ...
                self.log_out(
                    "<!> 图片自动切换时间间隔设置为 %d (秒)" % self.switch_interval,
                    level="WARNING",
                )
        ...
        def change_config():
            ask_config_file_path = askopenfilename(
                title="请选择配置文件",
            ...
            if exists(ask_config_file_path):
                ...
                def _not_ok():
                    ...
                    config_path_text_button.after(
                        ms=5000,
                        func=lambda: config_path_text_button.config(fg="#000000"),
                    )
        ...
        img_fast_mode_button = tkinter.Button(
            master=check_frame1, text="快速模式", command=fast_mode
        )
        img_fast_mode_button.pack(side=tkinter.LEFT, padx=5)
        img_hd_mode_button = tkinter.Button(
            master=check_frame1, text="高清模式", command=hd_mode
        )
        ...
        local_mode_button = tkinter.Button(
            master=check_frame2, text="本地模式", command=local_mode
        )
        local_mode_button.pack(side=tkinter.LEFT, padx=5)
        web_mode_button = tkinter.Button(
            master=check_frame2, text="网络模式", command=web_mode
        )
        ...
        tkinter.Button(
            ...
            text="查看历史图片",
            command=lambda: startfile(self.history_save_dir),
        ).pack(side=tkinter.TOP, padx=5)
        ...
        tkinter.Button(
            master=history_frame,
            text="确认",
            command=set_history_count_max,
        ).pack(side=tkinter.LEFT)
        history_frame.pack()

        # auto save
        auto_save_frame = tkinter.Frame(master=post_win)
        auto_save_label = tkinter.Label(
            master=auto_save_frame,
            text="自动保存",
            fg="green" if self.auto_save_do else "red",
        )
        auto_save_label.pack(side=tkinter.LEFT)
        auto_save_button = tkinter.Button(
            master=auto_save_frame,
            text="开启" if not self.auto_save_do else "关闭",
        ...
        bg_color_button = tkinter.Button(
            master=bg_color_frame, text="选择颜色", command=set_bg_color
        )
        ...
        config_path_text_button = tkinter.Text(
            master=change_config_frame,
        ...
        config_path_button = tkinter.Button(
            master=change_config_frame, text="选择配置文件", command=change_config
        )
        ...
        run_log_button = tkinter.Button(
            master=run_log_frame,
            text="关闭日志" if self.run_log else "开启日志",
            fg="green" if self.run_log else "red",
            command=set_run_log,
        )
        open_log_dir_button = tkinter.Button(
            master=run_log_frame,
            text="打开日志目录",
